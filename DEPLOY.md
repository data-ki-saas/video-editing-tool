...
3. **Auth settings** (Authentication > Providers > Email): disable "Confirm
   email" unless you've configured SMTP — otherwise sign-up will silently
   require a confirmation email that never arrives.
4. Collect from **Project Settings > API**:
   ...
   | `service_role` key (⚠️ secret — backend/worker/webhook only, never ship to the browser) | `<SUPABASE_SERVICE_ROLE_KEY>` |
...
3. **CORS mismatch causing "Failed to fetch" on asset upload.** Backend's
...
4. **Webhook secret mismatch.** `CREATOMATE_WEBHOOK_SECRET` is generated by
...
5. **Public custom domain on the wrong bucket.** If asset URLs ever look
...
6. **`R2_ENDPOINT_OVERRIDE` set in a real environment.** Backend-only, test
   suite-only. If set in Render's dashboard, R2 access silently points at
   the wrong (or no) host.
7. **Cold starts on Render's free tier.** Both the backend and the worker
...
8. **Worker transfer never happens.** Check three things in order: (a) the
