...
import { computeEffectiveCropRect, FULL_FRAME_CROP_RECT, type CropRect } from "@/lib/video/video_math";
import { IMAGE_TEMPLATE_OPTIONS, buildKenBurnsEffect, type ImageTemplateId } from "@/lib/video/imageTemplates";
import {
...
export function ImageTemplatesDialog({
   ...
   * (and the real clip, once added) animates relative to this, same as
  ...
  // Loops the chosen template's motion over the loaded image, redrawing
  // every frame -- reuses computeEffectiveCropRect/buildKenBurnsEffect
  // verbatim, the exact math the real added clip will use.
  useEffect(() => {
    const canvas = canvasRef.current;
    const ctx = canvas?.getContext("2d");
    if (!canvas || !ctx || !loadedImage) return;

    const zoomEffect = buildKenBurnsEffect(templateId, base, 0, durationSeconds);
    let rafId: number;
    ...
    function draw(timestamp: number) {
      ...
      const crop = computeEffectiveCropRect(base, [zoomEffect], elapsed);
  ...
  }, [loadedImage, templateId, durationSeconds, base]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Panel 1 (20%) -- photo picker. */}
        <div style={{ flexBasis: "20%" }} className="mb-3 flex min-h-0 shrink-0 flex-col gap-1.5">
        ...
        {/* Panel 2 (30%) -- Ken Burns template picker. */}
        <div style={{ flexBasis: "30%" }} className="mb-3 flex min-h-0 shrink-0 flex-col gap-1.5">
          <p className="text-[11px] text-muted">Choose an animation</p>
        ...
        {/* Panel 3 (50%) -- animated preview + duration control. */}
        <div style={{ flexBasis: "50%" }} className="flex min-h-0 flex-1 flex-col gap-2">
