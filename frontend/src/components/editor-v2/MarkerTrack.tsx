 ...
 * right-click to remove it. Purely a planning/organizational aid ("cut to
...
import { snapToNearest } from "@/lib/video/video_math";
...
export function MarkerTrack({
  ...
  return (
    <div
      ref={trackRef}
      onClick={handleClick}
      title="Click to drop a marker; drag one to move it, right-click to remove"
    ...
    >
      {markers.map((marker, index) => (
        <div
          ...
          onContextMenu={(e) => {
            e.stopPropagation();
            openContextMenu(e, [{ label: "Remove marker", danger: true, onSelect: () => onDelete(index) }]);
          }}
