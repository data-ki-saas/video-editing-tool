...
`FaceLandmarker` (the legacy `mediapipe.solutions.face_mesh` API this was
originally written against was removed as of mediapipe 1.0 -- see this
file's own git history) against the same 468/478-point face mesh topology
...
Unlike the legacy Solutions API, the Tasks API needs its model bundle
downloaded separately (it isn't packaged in the pip wheel) -- `_ensure_model`
lazily fetches Google's official `face_landmarker.task` bundle to a local
cache path on first use, rather than shipping a ~3.7MB binary in this repo.
A download failure (offline build box, blocked egress, transient network
issue) is caught by this file's own broad fallback the same as a
face-detection miss -- see analyze_photo's own doc comment.
"""
...
def analyze_photo(photo_bytes: bytes) -> FacePalette:
    ...
    detected, the model download hiccuped, or mediapipe itself failed to
    load in this environment; the atlas builder still produces a usable (if
    generic-toned) avatar either way."""
    try:
        import numpy as np
        ...
        options = FaceLandmarkerOptions(base_options=BaseOptions(model_asset_path=str(model_path)), num_faces=1)
